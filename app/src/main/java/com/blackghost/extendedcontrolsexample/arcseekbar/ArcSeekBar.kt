...
import com.blackghost.extendedcontrolsexample.R
...
class ArcSeekBar @JvmOverloads constructor(
    ...
    var maxProgress = a.useOrDefault(100) { getInteger(R.styleable.ArcSeekBar_maxProgress1, it) }
        set(progress) {
    ...
    var progress: Int = a.useOrDefault(0) { getInteger(R.styleable.ArcSeekBar_progress1, it) }
        set(progress) {
    ...
    var progressWidth: Float = a.useOrDefault(4 * context.resources.displayMetrics.density) { getDimension(
        R.styleable.ArcSeekBar_progressWidth, it) }
        set(value) {
            field = value
    ...
    private val thumb: Drawable = a?.getDrawable(R.styleable.ArcSeekBar_thumb1) ?: resources.getDrawable(
        R.drawable.ic_thumb_3
    )
